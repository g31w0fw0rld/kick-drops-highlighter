...
| `test-campanas-dom-nuevo.js` | La pestaña de campañas con el DOM de **septiembre** (tokens `bg-surface-bg-*`), que hasta aquí solo se probaba en reclamados y cerradas: tres grupos a la vez, uno con las doce recompensas reclamadas —con la casilla se van sus baldosas y su «Recompensas disponibles», el grupo del juego se queda—, el emote y el emblema de las campañas de KICK con su coste en el panel y el ⏱ de lo más caro en la página, los filtros por juego de Kick **a la vista** en esta pestaña, y sin API el tramo reconstruido desde barras casi a cero (`0.0013333` → 1500 min, no 1501). Pasa con el script de hoy: es un guardarraíl, con sensibilidad comprobada contra una copia sin las clases nuevas y otra sin el redondeo a múltiplos de 5 |
| `test-campanas-reclamado-a-medias.js` | La sub-campaña reclamada **a medias** en el DOM de septiembre (Starseries, 5 tickets «Pedido» y 6 en curso): con la casilla se van los cinco y solo ellos, «Recompensas disponibles» se queda, la marca es «⏱ 28h 2m» (el ticket de 35 h), y con la API diciendo que no hay nada cobrado no se esconde nada —el «Pedido» de la baldosa no decide—. Sin API, el aviso repite el texto de Kick y el modal del ticket de 10 h dice `417 / 600 min` —lo visto es el tramo menos lo que Kick dice que falta, para que el «3h 3m» coincida con su baldosa—: es la barra más alta (70 %), y contra la 1.3.21 salía `422 / 605` porque se dividía por 1 − f el minuto que Kick redondea hacia arriba |
| `test-ficha-idiomas.js` | «Saber más» en los 16 idiomas, con el i18n real y las propias funciones del script: la ficha trae la fila **Idiomas** contada de `Object.keys(i18n)` —etiqueta en los 16, traducida y con los dos puntos de «Autor»—, ninguna descripción termina ya en «16 idiomas.», y el agrupado de dos en dos no deja una frase sola al final (el control, sin la guarda, la deja en 15 idiomas). Y abre la ficha de verdad: dice `Idiomas:16` |
