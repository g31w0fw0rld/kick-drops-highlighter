    ...
    window.addEventListener("load", () => {
        ...
        function _streakMeterRow() {
            ...
            if (dias === null) {
                ...
                // CON NUMERO, LA FILA ABRE EL MODAL DEL COFRE, igual que su baldosa en el
                // inventario. Pedido el 2026-09-23: el medidor habla del cofre —cuando se
                // reinicia, si esta a salvo— y el sitio donde Kick cuenta eso mismo es su
                // modal, asi que una fila que lo resume y no lleva a el se lee como rota.
                // Es el mismo camino que la baldosa (`_openDailyRewardModal`), que NO
                // reclama en ningun estado: solo abre, y en movil pasa por el menu de la
                // cuenta. Aqui no se cierra solo, al reves que en `_pedirRacha`: lo has
                // abierto para mirarlo.
                row.setAttribute('data-streak-open', '1');
                row.style.cursor = 'pointer';
                row.onclick = () => {
                    _hideTip();
        ...
        function createInventoryCheckboxes(inline = false) {
            ...
            const expiredCb = makeCheckbox('cb-hide-expired', t.hideExpired, cleanExpiredInventoryFlag, (checked) => {
                ...
                if (_isClaimedPage()) {
                    ...
                    _updateClaimedTilesHidden();
                } else if (_isExpiredPage() && checked) {
                    // Y en cerradas, por lo mismo: ahi tambien quedan botones de reclamar
                    // (ver _finishReviewOutsideClaimed). Sin esto, marcar la casilla
                    // estando en esa pestaña guardaba el ajuste y nada mas, y la
                    // recompensa se quedaba sin cobrar hasta recargar.
                    //
                    // Apagarla no lleva recarga ni devuelve nada: en cerradas el barrido
                    // no esconde, solo reclama.
                    _dropsReviewInProgress = true;
                    cleanInventory("claim", _finishDropsReview);
                }
            });
        ...
        function _infoParagraphs(text, perPara) {
            ...
            // Una frase sola al final se va con el parrafo anterior. Con un numero impar
            // de frases el agrupado la dejaba suelta, y desde la 1.3.0 la suelta era
            // «16 idiomas.» —en 15 de los 16 idiomas de Twitch—, un parrafo de dos
            // palabras que parecia un resto. Cada frase que se añade cambia la paridad,
            // asi que no vale con cuadrar el texto: lo arregla el agrupado.
            if (perPara > 1 && out.length > 1 && sentences.length % perPara === 1) {
                out[out.length - 2] += joiner + out.pop();
            }
        ...
        function showInfoModal() {
            ...
            [
                ...
                // Contada del propio i18n y no escrita a mano: vivia como frase final de la
                // descripcion, en los 16 textos, y el agrupado de dos en dos la dejaba sola.
                { label: t.scriptInfoLanguages, value: String(Object.keys(i18n).length) },
        ...
        // El coste del tramo a partir de la barra (`f`) y de los minutos que faltan segun
        // el texto de Kick (`faltan`), para cuando no hay API.
        //
        // Dividir a secas —`faltan / (1 - f)` redondeado a multiplos de 5, que es lo que
        // se hacia hasta la 1.3.21— falla con la barra alta, porque Kick escribe lo que
        // falta redondeado HACIA ARRIBA: en el volcado de Starseries (fixture
        // `campaigns-partial-claimed`) el ticket de 10 h va al 0.69666666, o sea
        // 600 × 0.30333334 = 182.000004 min, y el texto dice «3 h y 3 min». Ese minuto de
        // mas, dividido por 0.30, son 3.3 minutos de total: 603.3, que el redondeo manda a
        // 605. A partir de ~60 % de barra un solo minuto ya se sale del redondeo.
        //
        // Asi que se hace al reves: se buscan los tramos que harian escribir a Kick
        // EXACTAMENTE ese texto, y de ellos el mas redondo, porque los tramos de Kick son
        // horas, medias horas o cuartos. «Exactamente» es literal: los seis textos del
        // volcado son `ceil(tramo × (1 − f))` con el MISMO `f` de la barra, ruido de coma
        // flotante incluido (182.000004 → 183), asi que esa es la condicion. El 1e-7 solo
        // absorbe el ruido de la resta en doble (1500 × 0.996 = 1494.0000000000002).
        // Con la barra alta caben varios (al 98 %, «3 min» admiten cualquier tramo entre
        // 110 y 165), y ahi desempata lo visto: es un numero ENTERO de minutos
        // —`progress_units` lo es, y en el volcado 0.69666666 × 600 = 417.999996, o sea
        // 418—, asi que se prefiere el tramo que deja lo visto entero. Luego lo redondo.
        // Barrido con todos los tramos multiplos de 5 min hasta 50 h y lo visto a saltos de
        // 3 min (300.500 casos), con `f` en float32 como la manda Kick: acierta en todos.
        // La division de antes, en el mismo barrido con tramos de 15 min, fallaba uno de
        // cada cinco.
        //
        // Si no cabe ninguno —un texto que solo dice horas, «5 h», no tiene precision de
        // minuto— se queda el redondeo de antes.
        function _kickTierFromBar(faltan, f) {
            const resto = 1 - f;
            const desde = Math.ceil((faltan - 1) / resto);
            const hasta = Math.floor((faltan + 1) / resto);
            const encaja = c => Math.ceil(c * resto - 1e-7) === faltan;
            // El error de `f` en float32 es de ~6e-8, que en lo visto son c × 6e-8 minutos.
            const vistoEntero = c => Math.abs(f * c - Math.round(f * c)) <= Math.max(1e-4, c * 2e-7);
            const centro = (faltan - 0.5) / resto;
            const elegir = (acepta) => {
                for (const paso of [60, 30, 15, 5]) {
                    let mejor = null;
                    for (let c = Math.ceil(desde / paso) * paso; c <= hasta; c += paso) {
                        if (!encaja(c) || !acepta(c)) continue;
                        if (mejor === null || Math.abs(c - centro) < Math.abs(mejor - centro)) mejor = c;
                    }
                    if (mejor !== null) return mejor;
                }
                return null;
            };
            const tramo = elegir(vistoEntero);
            if (tramo !== null) return tramo;
            const redondo = elegir(() => true);
            if (redondo !== null) return redondo;
            return Math.round((faltan / resto) / 5) * 5;
        }
        ...
        function _resolveKickProgress(li, rewardName) {
            ...
            //
            // El total no sale de dividir a secas: el texto de Kick viene redondeado, y
            // con la barra alta ese redondeo se multiplica (ver `_kickTierFromBar`). Lo
            // que NO se toca es lo que falta: ese numero es el que Kick enseña y el que
            // el usuario va a comparar con el aviso.
            //
            ...
            if (!campaign && !parsed.isTotal) {
                ...
                const total = _kickTierFromBar(parsed.minutes, f);
        ...
        //
        // AQUI DECIA QUE KICK NO TRADUCE EL `aria-label`, Y ERA FALSO. La nota venia del
        // desarrollo original —«el DOM estaba en español y el atributo seguia en ingles»—
        // y el 2026-09-13 se vio el DOM de verdad con Kick en español:
        //
        //     aria-label="Reclamar recompensa de Slouched Emote"  ·  <div>Pedir</div>
        //
        // O sea que la nota describia UN VOLCADO en ingles, no el comportamiento de Kick.
        // El fallo se vio en la pestaña de cerradas —donde el `aria-label` era la unica
        // puerta y no se reclamaba nada— pero afecta a las tres: en campañas el reclamo
        // seguia funcionando por el criterio 2, o sea de rebote.
        //
        // La señal que manda pasa a ser ESTRUCTURAL: `bg-primary-base`, el boton primario
        // de Kick, que no depende del idioma. El `aria-label` se sigue mirando primero
        // porque cuando esta en ingles es mas especifico, pero ya no sostiene nada solo.
        //
        ...
        //
        // Se rellena SOLO con lo visto en el sitio. Van los dos que hay verificados —«pedir»
        // del DOM en español del 2026-09-13 y «claim» del volcado en ingles— y los otros
        // catorce siguen sin inventar. Que la lista este a medias ya no deja a nadie sin
        // reclamar: lo que decide es `bg-primary-base`, y esto es el ultimo recurso.
        // Un texto por idioma, en minusculas, en el mismo orden que i18n y que
        // CLAIMED_TEXTS:
        //   es, en, de, fr, pt, ru, tr, ja, ko, pl, fi, vi, zh, ar, hi, id
        const CLAIM_BUTTON_TEXTS = ["pedir", "claim"];
         ...
         */
        // Los botones sobre los que se puede actuar dentro de un drop. Vive aparte porque
        // lo comparten las DOS puertas de entrada al reclamo: la barra completa y, en
        // cerradas, la etiqueta del boton (ver findClaimButtonByLabel).
        function _actionableButtonsIn(item) {
            if (!item) return [];
            return Array.from(item.querySelectorAll('button')).filter(btn => {
        ...
        // EL BOTON QUE SE DECLARA, SIN BARRA QUE LO RESPALDE.
        //
        // En la pestaña de cerradas no hay ni una `[role="progressbar"]` —cero en el
        // volcado `docs/dom-expired-claimable-2026-09.html`—, asi que la señal que abre
        // el reclamo en las otras pestañas no existe ahi: el progreso se acabo con la
        // campaña. Lo que si queda es la recompensa que te ganaste y no cobraste, con su
        // boton delante y sin barra ninguna:
        //
        //     en  <button class="… bg-primary-base …" dir="ltr"
        //                 aria-label="Claim Slouched Emote reward"><div>Claim</div>
        //     es  <button class="… bg-primary-base …" dir="ltr"
        //                 aria-label="Reclamar recompensa de Slouched Emote"><div>Pedir</div>
        //
        // SON DOS SEÑALES Y LA QUE MANDA ES LA CLASE, no la etiqueta. Se probo primero con
        // el `aria-label` —esta seccion aseguraba que Kick no lo traduce— y el 2026-09-13
        // el usuario mando el boton de arriba, el de su propia pagina en español: la
        // etiqueta SI se traduce, asi que en español no se reclamaba nada.
        //
        // `bg-primary-base` es el boton primario de Kick y en toda esa pagina aparece UNA
        // sola vez: justo aqui (contado sobre el volcado). Es estructural, asi que vale
        // en los 16 idiomas, y ademas es la misma marca por la que este script ya reconoce
        // el boton de cobrar del cofre diario (`dialog.querySelector('button.bg-primary-base')`).
        // La etiqueta se sigue mirando primero porque cuando esta es mas especifica.
        //
        // El ambito lo estrecha el `<li>` de la recompensa, y ahi no hay mas botones: el
        // otro con etiqueta —«Watch to redeem»— cuelga de la cabecera del grupo, y las
        // demas recompensas no traen boton ninguno (15 con el texto «Claimed» y 11 con
        // «This reward is no longer available»).
        //
        // Aqui NO vale el criterio 2 de findClaimButtonInDropItem —«si la barra esta
        // completa y hay un solo boton accionable, es el de reclamar»—, porque lo que lo
        // sostiene es justo la barra que aqui falta: sin ella un boton solitario puede
        // ser participar, conectar o cualquier cosa que Kick añada, y el click errado es
        // peor que no reclamar. En cerradas se exige una señal de las dos y nada mas.
        //
        // Un «Claimed» que algun dia viniera como boton en vez de como texto no se cuela:
        // vendria deshabilitado y `_actionableButtonsIn` ya lo descarta.
        const SEL_CLAIM_PRIMARY = '.bg-primary-base';

        // Se avisa UNA vez por sesion: un boton en una recompensa de campaña cerrada que
        // no case con ninguna de las dos señales es exactamente el fallo de arriba
        // repitiendose con otra marca, y sin esto vuelve a verse como «no hace nada».
        let _avisoCerradasSinSenal = false;

        function findClaimButtonNoBar(item) {
            const botones = _actionableButtonsIn(item);
            if (botones.length === 0) return null;
            const porEtiqueta = botones.find(btn => btn.matches('[aria-label*="claim" i]'));
            if (porEtiqueta) return porEtiqueta;
            const primario = botones.find(btn => btn.matches(SEL_CLAIM_PRIMARY));
            if (primario) return primario;
            if (!_avisoCerradasSinSenal) {
                _avisoCerradasSinSenal = true;
                console.warn('[Kick Drops] recompensa de campaña cerrada con boton sin señal reconocible:',
                    botones.map(b => b.getAttribute('aria-label') || (b.textContent || '').trim()));
            }
            return null;
        }
        ...
        function findClaimButtonInDropItem(item) {
            ...
            const candidates = _actionableButtonsIn(item);
            ...
            // 2. El boton primario de Kick, que no se traduce. Va antes del descarte
            //    porque es una señal positiva y no una deduccion (ver SEL_CLAIM_PRIMARY).
            const primario = candidates.find(btn => btn.matches(SEL_CLAIM_PRIMARY));
            if (primario) return primario;

            // 3. Por descarte: si la barra del drop esta completa y hay un unico boton
            //    accionable, ese es el de reclamar. Es lo que sostuvo el reclamo en
            //    español mientras el criterio 1 fallaba en silencio.
            if (candidates.length === 1) return candidates[0];

            // 4. Respaldo por texto (solo los idiomas verificados en el sitio).
        ...
        function cleanInventory(type = "expired", onDone = null) {
            ...
            // sin que nadie lo cancele, asi que si cambias de pestaña dentro de esa ventana
            // el barrido sigue corriendo con las ordenes de la pestaña anterior. Y no era
            // inocuo: `doHide` se calcula como
            //
            //     isTrophyCase ? false : (type === "expired")
            //
            // donde `isTrophyCase` se recalcula cada vuelta y exige `_isClaimedPage()`. En
            // reclamados sale true y por eso ahi no esconde nada; en cuanto te vas, pasa a
            // false, `doHide` se queda en true y en la pestaña NUEVA se ejecuta
            // `if (isClaimed && doHide) li.style.display = 'none'`. Reportado el 2026-09-04
            // en expiradas —«a veces oculta los drops reclamados»— y el «a veces» es la
            // ventana: tardando mas de nueve segundos en cambiar, el barrido ya habia muerto.
            // Pasa igual en proximas, que es la otra pestaña sin progreso ni reclamacion.
            //
            // Asi que el barrido se muere al salir de su pestaña, que es la unica lectura
            // coherente: sus ordenes describian la pagina que ya no tienes delante.
            const tabAlArrancar = _kindOfPath();
            // Se congela con la pestaña, y por el mismo motivo que ella: describe la
            // pagina que se tenia delante al arrancar el barrido.
            const enCerradas = (tabAlArrancar === 'expired');
            ...
            const checker = setInterval(() => {
                ...
                // escondido ni reclamado. `finish()` se llama igual para no dejar la revision
                // marcada como en curso para siempre —de eso cuelga el cofre, que espera a
                // que termine—.
                if (_kindOfPath() !== tabAlArrancar) {
                ...
                campaignContainers.forEach(function (container) {
                    ...
                    dropItems.forEach(function (li) {
                        ...
                        if (progressBar) {
                            ...
                            // no lo que ya te ganaste, que sigue cobrandose desde ahi (ver
                            // _finishReviewOutsideClaimed). La señal es el `aria-label`, y
                            // solo esa (ver findClaimButtonNoBar).
                            if (!isClaimed && doClaim && enCerradas) {
                                const btnSinBarra = findClaimButtonNoBar(li);
                                if (claimDropButton(btnSinBarra, claimIndex)) {
                                    claimIndex++;
                                    hasClaimableButton = true;
                                    // Queda dicho en consola: es lo unico que distingue
                                    // «no encontro el boton» de «no llego a mirar».
                                    console.log('[Kick Drops] cerradas: reclamando',
                                        btnSinBarra.getAttribute('aria-label') ||
                                        (btnSinBarra.textContent || '').trim());
                                }
                            }
                            if (!isClaimed) {
        ...
        //
        // Y EN CERRADAS TAMBIEN SE RECLAMA, que es lo que cambio el 2026-09-13. El
        // barrido no pasaba por ahi porque una campaña cerrada se daba por perdida, y no
        // lo esta: lo que caduca es el PROGRESO, no lo que ya te ganaste. Lo dice la
        // propia pestaña, en la frase que Kick imprime encima de la lista —«These
        // campaigns have ended, so you can no longer earn progress on them. Rewards you
        // fully unlocked before they closed are still claimable»— y se comprobo dejando
        // una recompensa sin cobrar a proposito hasta que la campaña cerro: el boton
        // sigue ahi y sigue funcionando.
        //
        // Va en modo "claim" y no "expired" por lo mismo que en campañas: esconder
        // campañas cerradas en la pestaña de cerradas no es lo que pide la casilla, y el
        // ocultado ademas se lleva por delante lo unico que hay que mirar ahi. Asi que
        // de las dos mitades de la casilla, en esta pestaña solo actua la de reclamar.
        function _finishReviewOutsideClaimed() {
            if (_isCampaignsPage() || _isExpiredPage()) {
